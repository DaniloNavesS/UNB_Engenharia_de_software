...
No *inserir(No *h, int valor, char alimento[]) {
  ...
  if (h->dir != NULL && h->dir->cor == VERMELHO &&
      (h->esq == NULL || h->esq->cor == PRETO))
    h = rotacaoEsquerda(h);
  if (h->esq != NULL && h->esq->cor == VERMELHO && h->esq->esq != NULL &&
      h->esq->esq->cor == VERMELHO)
    h = rotacaoDireita(h);
  if (h->esq != NULL && h->esq->cor == VERMELHO && h->dir != NULL &&
      h->dir->cor == VERMELHO)
    inverterCores(h);
